...
//             t = BoxIntersectionTest(geom, path_segment.ray, temp_intersection);
//         } else if (geom.type == GeometryType::Sphere) {
//             t = SphereIntersectionTest(geom, path_segment.ray, temp_intersection);
//         } else if (geom.type == GeometryType::GLTF_Primitive) {
// #if USE_BVH
//             t = IntersectBVH(geom, bvh_nodes, tri_indices, vertices, indices, path_segment.ray, temp_intersection);
// #else
//             t = NaivePrimitiveIntersection(geom, vertices, indices, path_segment.ray, temp_intersection);
// #endif
//         }
...
//             intersect_point = temp_intersection.pos;
//             normal = temp_intersection.normal;
//             uv = temp_intersection.uv;
//         }
//     }
...
//         intersections[path_index].pos = path_segment.ray(t_min);
...
//         path_segments[index].color_accum += path_segments[index].throughput * glm::vec3(1.0f);
//         path_segments[index].remaining_bounces = 0;
//         return;
//     }

// #if DEBUG_UV
//     path_segments[index].color_accum = glm::vec3(intersection.uv, 0.0f);
//     path_segments[index].remaining_bounces = 0;
// #elif DEBUG_NORMAL
//     path_segments[index].color_accum = glm::vec3(intersection.normal);
//     path_segments[index].remaining_bounces = 0;
// #elif DEBUG_DEPTH
...
//     path_segments[index].color_accum = glm::vec3(intensity);
//     path_segments[index].remaining_bounces = 0;
// #else
...
//         glm::vec3 radiance = material.mat.light.base_color * 5.0f;
...
//         float cos_out = glm::dot(w_out, intersection.normal);
//         float cos_in = glm::dot(sample.w_in, intersection.normal);
//         // if (sample.type & BSDFSampleType::Transmission && cos_in * cos_out > 0.0f) {
//         //     path_segments[index].remaining_bounces = 0;
//         // }

//         // if (sample.type & BSDFSampleType::Reflection && cos_in * cos_out < 0.0f) {
//         //     path_segments[index].remaining_bounces = 0;
//         // }
...
//             path_segments[index].remaining_bounces = 0;
//         } else {                
//             path_segments[index].throughput *= sample.bsdf * fabsf(cos_in) / sample.pdf;
...
__global__ void ShadeMaterial(
    ...
    Intersection intersection = shadable_intersections[index];
    if (intersection.t <= 1e-6) {
        path_segments[index].color_accum += path_segments[index].throughput * glm::vec3(0.4f);
        path_segments[index].remaining_bounces = 0;
        return;
    }
